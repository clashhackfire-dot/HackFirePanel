...
//   wrangler deploy
//
// Master key defaults to "admin" if never set. Change it any time from the
// dashboard's Settings > Security panel — it's stored in KV from then on.
// Optionally you can still set a `wrangler secret put MASTER_KEY` as the
// initial value instead of the "admin" fallback, but it's no longer required.
...
async function getMasterKey(env) {
  const stored = await env.USERS_KV.get('settings:masterKey');
  return stored || env.MASTER_KEY || 'admin';
}
...
async function handleApi(request, env, url) {
  ...
  if (url.pathname === '/api/login' && request.method === 'POST') {
    ...
    const masterKey = await getMasterKey(env);
    return jsonRes({ ok: body.key === masterKey });
  }
  ...
  // /api/settings/master-key  (POST change key — already authed above)
  if (url.pathname === '/api/settings/master-key' && request.method === 'POST') {
    const body = await safeJson(request);
    if (!body.newKey || body.newKey.length < 4) {
      return jsonRes({ error: 'New key must be at least 4 characters' }, 400);
    }
    await env.USERS_KV.put('settings:masterKey', body.newKey);
